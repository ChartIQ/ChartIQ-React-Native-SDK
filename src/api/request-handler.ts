...
import { ChartIQ, ChartIQDatafeedParams } from '@chartiq/react-native-chartiq';
...
const handleRequest = async (input: Omit<ChartIQDatafeedParams, 'id'>, session: string) => {
  const params = {
    isin: input.symbol.replace('^', ''),
    enddate: input.end ? moment(input.end).format('YYYYMMDDHHmm') : moment().format('YYYYMMDDHHmm'),
    startdate: input.start ? moment(input.start).format('YYYYMMDDHHmm') : moment().format('YYYYMMDDHHmm'),
    interval: input.period + (input.interval == 'minute' ? 'm' : input?.interval)
  };
  console.log('handleRequestParams::', params);
